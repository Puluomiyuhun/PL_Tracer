...
namespace osc {
  ...
  void SampleRenderer::createTextures()
  {
      ...
      //launchParams.envmap = textureObjects[numTextures];
  }
  OptixTraversableHandle SampleRenderer::buildAccel()
  {
      ...
      texcoordBuffer.resize(model->meshes.size());
  ...
  void SampleRenderer::buildSBT()
  {
    ...
    for (int i=0;i<missPGs.size();i++) {
      ...
      rec.envmap = textureObjects[(int)model->textures.size()];
    ...
    for (int meshID = 0; meshID < numObjects; meshID++) {
        for (int rayID = 0; rayID < RAY_TYPE_COUNT; rayID++) {
            ...
            if (rec.data.mat_mes.diffuseTextureID != -1) {
                rec.data.mat_mes.diffuse_texture = textureObjects[rec.data.mat_mes.diffuseTextureID];
            }
            if (rec.data.mat_mes.emitterTextureID != -1) {
                rec.data.mat_mes.emitter_texture = textureObjects[rec.data.mat_mes.emitterTextureID];
            }
  ...
  void SampleRenderer::resize(const vec2i& newSize)
  {
      ...
      launchParams.frame.renderBuffer = (float4*)renderBuffer.d_pointer();
  ...
  {
      renderBuffer.download(h_pixels,
